<?php
	...
	$rewrite_rules = array(
		'/^\/?$/i'										=>	ABSPATH . ADMIN_PATH . 'sources' . DIRECTORY_SEPARATOR . 'home.php',
	);
	...
	if (empty($rewritten_url)) {
		...
		template_die("<h1>404 File Not Found</h1><br />The requested page <em>" . htmlspecialchars('/' . $request_uri) . "</em> could not be found.");
	}
	...
	# File we've rewritten to doesnt exist. Show 404 error
		...
		# Show human readable error message
		template_die("<h1>404 File Not Found</h1><br />The requested page <em>" . htmlspecialchars('/' . $request_uri) . "</em> could not be found.<br />If you've recently upgraded make sure you uploaded all the necessary files.");
	}
